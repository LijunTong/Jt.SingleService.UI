<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column prop="isAuthor" label="是否作者 0：不是，1：是" />
    ...
    >
      ...
      >
        <el-form-item label="评论id" prop="commentId">
          <el-select v-model="formData.commentId" placeholder="请选择"><el-option v-for="item in [1,2,3]" :key="item" :label="item" :value="item" /></el-select>
        </el-form-item>
      ...
      <el-descriptions
        ...
        :column="2"
      >
        <el-descriptions-item label="评论id">{{ formData.commentId }}</el-descriptions-item>
        ...
        <el-descriptions-item label="是否作者 0：不是，1：是">{{ formData.isAuthor }}</el-descriptions-item>
...
<script>
import * as api from '@/api/comment_like'
export default {
  name: 'CommentLike',
  data() {
    return {
      ...
      rules: {
        commentId: [{ required: true, message: '评论id不能为空', trigger: 'blur' }],
