<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column prop="parentId" label="回复id" />
      <el-table-column prop="replyTo" label="回复谁" />
      <el-table-column
        prop="isTop"
    ...
    >
      ...
      >
        ...
        <el-form-item label="回复id" prop="parentId">
          <el-select
        ...
        <el-form-item label="回复谁" prop="replyTo">
          <el-input v-model="formData.replyTo" />
        </el-form-item>
        <el-form-item label="是否置顶 0：不置顶，1：置顶" prop="isTop">
          <el-input-number v-model="formData.isTop" />
        </el-form-item>
        <el-form-item label="是否作者 0：不是，1：是" prop="isAuthor">
          <el-input-number v-model="formData.isAuthor" />
        </el-form-item>
      ...
      >
        ...
        <el-descriptions-item label="回复id">{{
          formData.parentId
        }}</el-descriptions-item>
        <el-descriptions-item label="回复谁">{{
          formData.replyTo
        }}</el-descriptions-item>
        <el-descriptions-item label="是否置顶 0：不置顶，1：置顶">{{
          formData.isTop
        }}</el-descriptions-item>
        <el-descriptions-item label="是否作者 0：不是，1：是">{{
          formData.isAuthor
        }}</el-descriptions-item>
...
<script>
import * as api from '@/api/article_comment'
export default {
  name: 'ArticleComment',
  data() {
    return {
      ...
      rules: {
        ...
        parentId: [{ required: true, message: '回复id不能为空', trigger: 'blur' }],
        replyTo: [{ required: true, message: '回复谁不能为空', trigger: 'blur' }],
        isTop: [{ required: true, message: '是否置顶 0：不置顶，1：置顶不能为空', trigger: 'blur' }],
        isAuthor: [{ required: true, message: '是否作者 0：不是，1：是不能为空', trigger: 'blur' }],
