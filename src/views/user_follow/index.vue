<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column prop="userId" label="标签id" />
      <el-table-column prop="followId" label="文章id" />
    ...
    >
      <el-form
        ...
        label-width="100px"
      >
        <el-form-item label="标签id" prop="userId">
          <el-select v-model="formData.userId" placeholder="请选择"><el-option v-for="item in [1,2,3]" :key="item" :label="item" :value="item" /></el-select>
        </el-form-item>
        <el-form-item label="文章id" prop="followId">
          <el-select v-model="formData.followId" placeholder="请选择"><el-option v-for="item in [1,2,3]" :key="item" :label="item" :value="item" /></el-select>
        </el-form-item>
        <el-form-item label="关注时间" prop="followTime">
          <el-date-picker v-model="formData.followTime" type="datetime" placeholder="选择日期时间" />
        </el-form-item>
      ...
      <el-descriptions
        ...
        :column="2"
      >
        <el-descriptions-item label="标签id">{{ formData.userId }}</el-descriptions-item>
        <el-descriptions-item label="文章id">{{ formData.followId }}</el-descriptions-item>
        <el-descriptions-item label="关注时间">{{ formData.followTime }}</el-descriptions-item>
...
<script>
import * as api from '@/api/user_follow'
export default {
  name: 'UserFollow',
  data() {
    return {
      ...
      rules: {
        userId: [{ required: true, message: '标签id不能为空', trigger: 'blur' }],
        followId: [{ required: true, message: '文章id不能为空', trigger: 'blur' }],
        followTime: [{ required: true, message: '关注时间不能为空', trigger: 'blur' }],
