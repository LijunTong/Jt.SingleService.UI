<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column prop="isCollect" label="是否收藏 0：不是，1：是" />
    ...
    >
      ...
      >
        ...
        <el-form-item label="收藏时间" prop="collectTime">
          <el-date-picker v-model="formData.collectTime" type="datetime" placeholder="选择日期时间" />
        </el-form-item>
        ...
        <el-form-item label="是否收藏 0：不是，1：是" prop="isCollect">
          <el-input-number v-model="formData.isCollect" />
        </el-form-item>
      ...
      >
        ...
        <el-descriptions-item label="收藏时间">{{ formData.collectTime }}</el-descriptions-item>
        <el-descriptions-item label="作者id">{{ formData.userId }}</el-descriptions-item>
        <el-descriptions-item label="是否收藏 0：不是，1：是">{{ formData.isCollect }}</el-descriptions-item>
...
<script>
import * as api from '@/api/article_collect'
export default {
  name: 'ArticleCollect',
  data() {
    return {
      ...
      rules: {
        ...
        collectTime: [{ required: true, message: '收藏时间不能为空', trigger: 'blur' }],
        userId: [{ required: true, message: '作者id不能为空', trigger: 'blur' }],
        isCollect: [{ required: true, message: '是否收藏 0：不是，1：是不能为空', trigger: 'blur' }],
